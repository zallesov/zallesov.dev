...
export default function CVPage({ cv }: Props) {
  return (
    <main className="dotted-overlay flex min-h-screen flex-col items-center justify-between py-12 lg:px-12 sm:px-2 bg-gradient-custom bg-fixed">
      ...
      {cv.sideProjects && (
        <section className="py-10 md:py-16 w-full">
          <SideProjectsSection
            projects={cv.sideProjects}
            quote={cv.sideProjectsQuote}
          />
        </section>
      )}
      ...
      {cv.portfolios && (
        <section className="py-10 md:py-16">
          <PortfoliosSection
            ...
            portfolioQuote={cv.portfolioQuote}
          />
        </section>
      )}
